...
function generate_simulation_for_player () {
  ...
  if [ $simulationSuccess -eq 0 ]; then
      ...
      curl --location --request GET 'https://'$LAMBDA_API_ID_1'.execute-api.us-east-1.amazonaws.com/'$LAMBDA_API_NAME_1'?account_id='$ACCOUNTID'&ftype=getSummary'
      curl --location --request GET 'https://'$LAMBDA_API_ID_1'.execute-api.us-east-1.amazonaws.com/'$LAMBDA_API_NAME_1'?account_id='$ACCOUNTID'&event_id='$EVENTID'&ftype=GetSingleEvent'
      curl --location --request GET 'https://'$LAMBDA_API_ID_1'.execute-api.us-east-1.amazonaws.com/'$LAMBDA_API_NAME_1'?account_id='$ACCOUNTID'&event_id='$EVENTID'&ftype=GetLabeledImage'
